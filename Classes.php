...
//clsFileUpload Class @0-2C1DB08F
class clsFileUpload
{
...
class clsLocale {
  ...
  function LoadTranslation($filename = "") {
    ...
    $this->Messages["ccs_charset"] = "utf-8";
...
//clsCustomCacheManager class @0-CA0B636F
class clsCustomCacheManager {

  function clsCustomCacheManager() {
  }

  function IsDisabled ($parameters) {
    foreach ($parameters as $parameter) {
      if ($parameter["Target"] == "Disable") {
        $param = "";
        switch($parameter["Source"]) {
          case "URL": 
            $param =  CCGetFromGet($parameter["Name"]);
            break;
          case "Form": 
            $param = CCGetFromPost($parameter["Name"]);
            break;
          case "Session": 
            $param = CCGetSession($parameter["Name"]);
            break;
          case "Expression": 
            $param = $parameter["Name"];
            break;
        }
        if ($parameter["Target"] == "Disable"  && $param != "" )
          return true;
      }
    }
    return false;
  ...
  function GetCacheKey($page, $parameters) {
    global $CCSLocales;
    ...
    $Session = "";
    foreach ($parameters as $parameter) {
      if ($parameter["Target"] == "Key")
        switch($parameter["Source"]) {
          case "URL": 
            $Get .= ";" . $parameter["Name"] . "=" . CCGetFromGet($parameter["Name"]);
            break;
          case "Form": 
            $Post .= ";" . $parameter["Name"] . "=" . CCGetFromPost($parameter["Name"]);
            break;
          case "Session": 
            $Get .= ";" . $parameter["Name"] . "=" . CCGetSession($parameter["Name"]);
            break;
          case "Expression": 
            $Get .= ";" . $parameter["Name"];
            break;
    ...
    $key = "GET:" . substr($Get, 1) . ":POST:" . substr($Post, 1) . ":Session:" . substr($Session, 1) . ":Locale:" . $CCSLocales->Locale . ":Style:" . $CCProjectStyle;
    return md5($page) . "." . md5($key);
  }
  ...
  function PutObject($key, $object, $duration) {
  }
  ...
  function ClearStartedWith($PageKey) {
  }
...
//End clsCustomCacheManager class

//clsMainPage Class @0-81983608
class clsMainPage
{
...
//clsFlashChart class @0-AEC56BC6
class clsFlashChart {
  public $Name;
  public $Title = "";
  public $Width = 640;
